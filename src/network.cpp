...
struct IOCP
{
	...
	void* load_extfunc(const Socket& socket, GUID& guid)
	{
		void* func = nullptr;
		DWORD bytes = 0;
		WSAIoctl(socket.handle(), SIO_GET_EXTENSION_FUNCTION_POINTER, 
			&guid, sizeof(guid), 
			&func, sizeof(func), 
	...
	bool post_oper(NetworkSession* session, NetworkOperation* oper)
	{
		...
		if(oper->type == OperationType::Accept)
		{
			...
			DWORD trans = 0;
			if(TRUE != this->acceptEx(session->socket, oper->acceptedSocket, 
				oper->wsabuf.buf, (DWORD)(oper->wsabuf.len - addrLen * 2), 
				(DWORD)addrLen, (DWORD)addrLen, 
				&trans, &oper->overlapped))
			{
	...
	DWORD exec_oper()
	{
		...
		for(;;)
		{
			...
			if (oper->type == OperationType::Accept)
			{
				...
				sockaddr_in* addrL = nullptr; int lenL = 0;
				sockaddr_in* addrR = nullptr; int lenR = 0;
							
				this->getAcceptExSockAddrs(
					oper->wsabuf.buf, dataLen, addrLen, addrLen,
					(sockaddr**)&addrL, &lenL, (sockaddr**)&addrR, &lenR);
